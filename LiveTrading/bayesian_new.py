...
class Strategy:
    """
    A Bayesian strategy with "trained" priors based on offline analysis of
    game data and price action. The model prioritizes the non-linear relationship
    between score difference and time remaining.

    Key features:
    - Informed Priors: The Bayesian model starts with weights derived from
      analyzing historical game/price data, focusing on time-scaled score
      difference and momentum.
    - Robust Risk Management: Combines a dynamic learning guard, position/order
      caps, cooldowns, and take-profit/stop-loss mechanisms to control volatility.
    - Adaptive Player Features: Includes features for player performance that
      are learned online without strong initial assumptions.
    """
    ...
    _EVENTS = ("SCORE","STEAL","TURNOVER","BLOCK","FOUL","MISSED",
               "SUBSTITUTION","TIMEOUT","END_PERIOD","END_GAME","REBOUND","JUMP_BALL")
    _SHOTS  = ("THREE_POINT","TWO_POINT","DUNK","LAYUP","FREE_THROW")
    _D = 1 + 2 + 4 + 3 + 2 + len(_EVENTS) + len(_SHOTS) + 2 + 5

    def reset_state(self) -> None:
        """
        Reset state and initialize the model with "trained" priors.
        The priors are set based on offline analysis of historical game data.
        """
        # --- Informed Priors ---
        # Feature indices for clarity:
        # 0: bias
        # 1-2: time features (t_norm, t_left)
        # 3-6: score features (sd, ad, sd_t, ad_t)
        # 7-9: momentum features (sd_chg_1, sd_mom_10, sd_mom_30)
        # 10-11: run/pace features
        # 12-23: event one-hot
        # 24-28: shot one-hot
        # 29-30: microstructure (spread, mid_dev)
        # 31-35: player features
        self.mu: List[float] = [0.0] * self._D
        self.mu[0] = 50.0  # Bias: Start at a 50/50 price.
        self.mu[5] = 50.0  # sd_t (time-scaled score diff): This is the most critical feature.
                           # A 5-point lead with 10% of game left (sd_t ≈ 0.5) should add ~$25
                           # to the fair value, matching observed market prices.
        self.mu[8] = 5.0   # sd_mom_10: Moderate positive prior for 10-event momentum.
        self.mu[9] = 2.5   # sd_mom_30: Smaller positive prior for 30-event momentum.
        # All other priors are 0.0, allowing the model to learn their impact online without strong bias.
        ...
        # --- Event & Player Buffers ---
        self._score_diffs: List[float] = []
        self._events_hist: List[str] = []
    ...
    # --- Bayesian Linear Regression & Utilities (omitted for brevity, same as previous) ---
    @staticmethod
    ...
    # --- Risk Management & Trading Logic Helpers (omitted for brevity, same as previous) ---
    ...
    def _kelly_target_shares(self, p_hat: float, s_prob: float, price_dollars: float, cash: float) -> float:
        ...
        edge = p - s
        f = (edge / (1.0 - s) if edge > 0 else edge / s) * self.KELLY_SCALE
    ...
    def on_game_event_update(self, event_type: str, home_away: str, home_score: int, away_score: int,
        ...
        if self._now_ms() - self.last_order_ms < self.TRADE_COOLDOWN_MS or abs(edge) < thr:
            return
        ...
        if spread <= self.MAX_CROSS_SPREAD:
            ...
            if side == Side.BUY:
                px = self.best_bid + self.PASSIVE_NUDGE if self.best_bid else mid - self.PASSIVE_NUDGE
            else:
                px = self.best_ask - self.PASSIVE_NUDGE if self.best_ask else mid + self.PASSIVE_NUDGE
