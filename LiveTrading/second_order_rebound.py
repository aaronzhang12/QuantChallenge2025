...
class Strategy:
    ...
    def on_game_event_update(self,
        ...
        if event_type == "SCORE" and self.entry_reason in ["FOUL_BONUS_TRADE", "OFFENSIVE_REBOUND_TRADE"]:
        ...
        if self.current_position == 0:
            if event_type == "REBOUND" and rebound_type == "OFFENSIVE":
                if home_away == "home":
                    print("TRADE SIGNAL: Buying due to Home Team offensive rebound.")
                    place_market_order(Side.BUY, Ticker.TEAM_A, self.TRADE_QUANTITY)
                    self.entry_reason = "OFFENSIVE_REBOUND_TRADE"
                else:
                    print("TRADE SIGNAL: Selling due to Away Team offensive rebound.")
        ...
        if event_type == "START_PERIOD":
            ...
            self.home_team_fouls, self.away_team_fouls = 0, 0
            self.home_team_in_bonus, self.away_team_in_bonus = False, False
            return

        if event_type == "END_GAME":
            if self.current_position != 0: self._close_position_at_market()
            self.reset_state()
            return
